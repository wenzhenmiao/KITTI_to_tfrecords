...
'''How to organize your dataset folder:
  VOCROOT/
       ...
       |    |->...
'''
tf.app.flags.DEFINE_string('dataset_directory', '/home/mx/data/KITTIdevkit/KITTI',
                           'All datas directory')
tf.app.flags.DEFINE_string('train_splits', 'KITTITRAIN',
                           'Comma-separated list of the training data sub-directory')
tf.app.flags.DEFINE_string('validation_splits', 'KITTIVAL',
                           'Comma-separated list of the validation data sub-directory')
tf.app.flags.DEFINE_string('output_directory', '/home/mx/data/KITTIdevkit/KITTI/KITTItfrecords',
...
def _process_image(filename, coder):
  ...
  with tf.gfile.FastGFile(filename, 'rb') as f:
    image_data = f.read()
  # Decode the RGB JPEG.
  image_png = coder.png_to_jpeg(image_data)
  image = coder.decode_jpeg(image_png)
...
def _find_image_bounding_boxes(directory, cur_record):
  ...
  """
  print('****************',cur_record)
  anna_file = os.path.join(directory, cur_record[0], 'Annotations', cur_record[1].replace('png', 'xml'))
  ...
  for obj in root.findall('object'):
      ...
      labels.append(int(dataset_common.KITTI_LABELS[label][0]))
...
def _process_dataset(name, directory, all_splits, num_shards):
  ...
  for split in all_splits:
    ...
    jpegs = [im_name for im_name in images if im_name.strip()[-3:]=='png']
